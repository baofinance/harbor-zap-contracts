...
if [[ -n "$MARKET" ]]; then
  DEPLOYMENT_FILE="deployments/mainnet/genesis-eth-zap-v4-${MARKET}-${DEPLOYMENT_TS}.json"
else
  DEPLOYMENT_FILE="deployments/mainnet/genesis-eth-zap-v4-${DEPLOYMENT_TS}.json"
fi
...
impl_out=$(deploy_contract "$IMPLEMENTATION_PATH" "$GENESIS_ETH")
