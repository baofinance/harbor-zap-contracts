...
if [[ -n "$MARKET" ]]; then
  DEPLOYMENT_FILE="deployments/mainnet/minter-eth-zap-v3-${MARKET}-${DEPLOYMENT_TS}.json"
else
  DEPLOYMENT_FILE="deployments/mainnet/minter-eth-zap-v3-${DEPLOYMENT_TS}.json"
fi
...
mapfile -t stability_pools < <(load_list ".markets[\"$MARKET\"].stabilityPools.eth[]?")
